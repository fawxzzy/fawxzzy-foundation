...
- Score Data and security posture: `warn` 10/25 - Wave 1A cleared the four search_path findings, Wave 1C cleared the SECURITY DEFINER execute-permission warnings, Wave 1B cleared the four unindexed foreign key findings, Wave 2A-1 removed the 12 session-core auth_rls_initplan warnings, Wave 2A-2 removed the 12 routine-core auth_rls_initplan warnings, and Wave 2A-3 removed the 12 profile/catalog auth_rls_initplan warnings, but leaked password protection is still disabled on the current free plan and the remaining follow-up-job RLS performance lane still blocks stronger privacy and data-security claims.
...
- Score warning: Remaining Fitness performance remediation lanes are now narrowed to auth_rls_initplan findings on session_follow_up_jobs, plus observation-only unused index review.
- Score warning: Historical prod-deploy mapping remains documented until active inventory or remediation changes.
- Score next action: Keep the leaked-password control recorded as a reviewed free-plan blocker unless Fitness crosses a launch/readiness gate, move the final session_follow_up_jobs auth_rls_initplan batch next, keep unused-index review observation-only, and require reviewed advisor proof before changing posture.
